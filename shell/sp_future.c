...
shellcmd sp_future(int nargs, char *args[]){	
	...
	*/
	...
	f_buffers = (SFdata **)getmem(sizeof(SFdata *)*(num_streams));
	i = 0;
	while(i < num_streams){
		SFdata *val = (SFdata *)getmem(sizeof(SFdata)*(work_queue));
		int j = 0;
		while(j < work_queue){
			val[j].f_ts = future_alloc(FUTURE_EXCLUSIVE, sizeof(int));
			val[j].f_d = future_alloc(FUTURE_EXCLUSIVE, sizeof(int));
			j++;
		}
		f_buffers[i] = val;
		i++;
